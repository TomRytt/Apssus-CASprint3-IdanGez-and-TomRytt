...
export class NoteEdit extends React.Component {
    ...
    editNoteInfo = (ev) => {
        ...
        console.log(type)
        if (type === "note-txt") noteService.updateNote(id, type, info.txt, info.style.backgroundColor)
            ...
            <div>
                ...
                <article className='mail-details-container'>
                    <form className="note-edit-form">
                        {type === "note-txt" && <input onChange={(ev) => this.handleChange(ev, "txt")} value={info.txt} placeholder='enter text here' type='text' />}
                        {type === "note-img" && <input onChange={(ev) => this.handleChange(ev, "label")} value={info.label} placeholder='enter img title here' type='text' />}
                        {(type === "note-img" || type === "note-video") && <input onChange={(ev) => this.handleChange(ev, "url")} value={info.url} placeholder='enter url here' type='text' />}
                        {type === "note-todos" && (
                            <div>
                                <label htmlFor={"label"}>Enter list label: </label>
                                <input onChange={(ev) => this.handleChange(ev, "label")} value={info.label} placeholder='enter list label here' type='text' id='label' />
                            </div>
                        )}
                        {type === "note-todos" &&
                            todos.map((t, idx) => {
                                return (
                                    <div key={`todo-container${idx}`}>
                                        ...
                                        <input key={`todo${idx}`} onChange={(ev) => this.handleChange(ev, "todos", idx)} value={todos[idx]} placeholder='enter todo here' type='text' id={`todo-${idx}`} />
                                    </div>
                                );
                            })}
                        {/* {ctg === "NoteTodos" && (
